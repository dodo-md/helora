...
class WidgetUpdateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.dodoznq.helora.ACTION_WIDGET_UPDATE_PLAYBACK_STATE") return
        ...
        scope.launch {
            try {
                ...
                val glanceIds = glanceAppWidgetManager.getGlanceIds(HeloraGlanceWidget::class.java)
                glanceIds.forEach { glanceId ->
                    HeloraGlanceWidget().update(context, glanceId)
                }
